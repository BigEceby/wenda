...
public interface MessageDAO {
    ...
    int conversationUnreadCount(@Param("conversationId") String conversationId,@Param("userId") int userId);

    @Update({"update",TABLE_NAME,"set has_read = 1 where conversation_id = #{conversationId} and to_id = #{userId}"})
    int changeConversationToRead(@Param("conversationId") String conversationId,@Param("userId") int userId);
